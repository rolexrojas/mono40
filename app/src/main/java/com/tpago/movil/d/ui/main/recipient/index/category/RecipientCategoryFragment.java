...
import com.tpago.movil.d.data.api.ApiService;
...
public class RecipientCategoryFragment
    ...
    public void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        final RecipientCategoryComponent component = DaggerRecipientCategoryComponent.builder()
                ...
                .recipientCategoryModule(new RecipientCategoryModule(category, (DepMainActivityBase) requireActivity()))
                .build();
    ...
    private void fetchPhoneNumber(Recipi) {
        api.fetchCustomer(this.queryPhoneNumber.formattedValued()).subscribeOn(Schedulers.io())
        ...
        if (item instanceof Recipient) {
            ...
            switch (((Action) item).type()) {
                case ADD_PHONE_NUMBER:
                    this.queryPhoneNumber = ((PhoneNumberAction) item).phoneNumber();
                    api.fetchCustomer(this.queryPhoneNumber.formattedValued()).subscribeOn(Schedulers.io())
                            .observeOn(AndroidSchedulers.mainThread())
                            .doOnSubscribe((d) -> this.takeoverLoader.show())
                            .doFinally(this.takeoverLoader::hide)
    ...
    private void handleError(Throwable throwable) {
        presenter.addRecipient(this.queryPhoneNumber, category);
    }

    private void handleResult(Result<Customer> customerResult) {
        Customer customer = customerResult.successData();
        if (customer != null) {
            presenter.addRecipient(new PhoneNumberRecipient(this.queryPhoneNumber, customer.getName()), category);
        } else {
            presenter.addRecipient(this.queryPhoneNumber, category);
        }
    }
