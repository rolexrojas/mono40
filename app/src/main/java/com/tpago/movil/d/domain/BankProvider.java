        ...
        remoteObservable = apiService.fetchBankSet()
          .map(new Function<Result<Set<Bank>, ApiCode>, Result<Set<Bank>, ErrorCode>>() {
            ...
            public Result<Set<Bank>, ErrorCode> apply(
              final Result<Set<Bank>, ApiCode> apiResult
            ) throws Exception {
              ...
              if (fetchedSet) {
                ...
                for (Bank bank : bankSet) {
                  picasso.load(bank.getLogoUri(LogoStyle.GRAY_20))
                    .fetch();
                  picasso.load(bank.getLogoUri(LogoStyle.GRAY_36))
                    .fetch();
                  picasso.load(bank.getLogoUri(LogoStyle.PRIMARY_24))
                    .fetch();
                  picasso.load(bank.getLogoUri(LogoStyle.WHITE_36))
                    .fetch();
                }
