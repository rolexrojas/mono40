 ...
 */
@Deprecated
public class TypeAdapterApiErrorDeprecated implements JsonDeserializer<ApiError> {
  ...
  public ApiError deserialize(
    ...
    if (!parent.has(PROPERTY_ERROR)) {
      ...
      if (!child.has(PROPERTY_CODE)) {
        ...
        return new ApiError(
          ApiError.Code.fromValue(child.get(PROPERTY_CODE)
            .getAsInt()),
          child.get(PROPERTY_DESCRIPTION)
            .getAsString()
        );
      }
    }
